...
- **Pipecat Voice Harness**: Pipecat is treated as an optional local voice sidecar harness, not as a replacement for TipTour's pointer/action engine. TipTour exposes a persisted "Pipecat Voice" connection toggle, connection status, and `PipecatVoiceHarnessClient` start/stop/health/event contract for a sidecar at `127.0.0.1:7860`. When enabled, the existing voice hotkey starts/stops the sidecar session instead of Gemini Live. The sidecar lives in `harnesses/pipecat-voice`, exposes TipTour/Hermes proxy tools, calls TipTour's localhost harness for observe/targets/actions, and may delegate long-horizon work to Hermes at `127.0.0.1:8642`; see `docs/pipecat-harness.md`.
...
| `TipTour/Voice/PipecatVoiceHarnessClient.swift` | ~165 | Local Pipecat sidecar contract. Provides health/start/stop/event probing, connection status modeling, and shared tool instructions for TipTour/Hermes interoperability. |
| `harnesses/pipecat-voice/server.py` | ~260 | FastAPI local Pipecat voice sidecar. Exposes `/v1/health`, `/v1/start`, `/v1/stop`, `/v1/events`, `/v1/tools`, and proxy tool calls into TipTour/Hermes while optionally detecting the installed Pipecat runtime. |
